...
rule run_generate_anchors_dictionary_for_ec_reads:
    ...
    shell:
        ...
        vg_anchor build --graph {input.sampled_pg_vg_ec} --index {input.subgraph_pg_dist_ec} \
...
rule run_get_anchors_from_gaf_for_ec_reads:
    ...
    shell:
        """
        vg_anchor get-anchors --dictionary {input.anchors_dictionary} --graph {input.sampled_pg_vg_ec} --alignment {input.chunked_gaf_ec} --fasta {input.chunked_fasta_ec} \
...
rule run_displayPafAlignments_for_r_utg_hifiasm_subregion_assembly_for_ec_reads:
	output:
		...
		plots_dir=directory("results_hs/hs-{k}/{sample_id}/{region_id}/ec/hifiasm_alignment/{asm_preset}_plots")
	input:
		displayPaf_bin=config["DISPLAYPAF"]["bin"],
		hg002_reference_chunked_for_hifiasm="results_hs/hs-{k}/{sample_id}/{region_id}/ec/hifiasm_assembly/hg002.chunked.{asm_preset}.for_hifiasm.fasta",
		r_utg_hifiasm_subregion_assembly="results_hs/hs-{k}/{sample_id}/{region_id}/ec/hifiasm_assembly/{sample_id}.{asm_preset}.hifiasm.r_utg.subregion.fasta",
	...
	params:
		asm_preset=config["MINIMAP"]["asmPreset"],
	benchmark: "benchmarks/{sample_id}/hs-{k}/{region_id}/run_displayPafAlignments_for_r_utg_hifiasm_subregion_assembly_for_ec_reads_{asm_preset}.benchmark.txt"
	log: "logs/{sample_id}/hs-{k}/{region_id}/run_displayPafAlignments_for_r_utg_hifiasm_subregion_assembly_for_ec_reads_{asm_preset}.log"
	shell:
		"""
		{input.displayPaf_bin} \
